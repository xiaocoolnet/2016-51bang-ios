...
typealias backForMp4func = (editedText:NSURL,images:UIImage)->Void


class IWVideoRecordingController: UIViewController,
AVCaptureFileOutputRecordingDelegate {
    
    var myFunc = backForMp4func?()
    
    //  最常视频录制时间，单位 秒
    let MaxVideoRecordTime = 15
    ...
    //  开始、停止按钮
    var startButton, stopButton,defineButton: UIButton!
    //  前后摄像头转换、闪光灯 按钮
...
//        self.boFangButton.backgroundColor = UIColor.redColor()
    ...
    func setupHeaderView() {
        ...
        //确认按钮
        defineButton = prepareButtons(btnTitle: "确认", btnSize: CGSizeMake(80, 40), btnCenter: CGPointMake(view.bounds.size.width-60, 40))
        defineButton.backgroundColor = UIColor.lightGrayColor()
        defineButton.userInteractionEnabled = false
        defineButton.addTarget(self, action: #selector(defineButtonAction), forControlEvents: .TouchUpInside)
        self.headerView.addSubview(defineButton)
        ...
        cameraSideButton.center = CGPoint(x: 80, y: centerY)
        ...
        totolTimeLabel.center = CGPoint(x: headerView.center.x-20, y: centerY)
        ...
        flashLightButton.center = CGPoint(x: headerView.bounds.width - 140, y: centerY)
    ...
    func onClickedEndButton(endButton: UIButton) {
        ...
        defineButton.userInteractionEnabled = true
        
        hiddenHeaderView(false)
        self.addBoFangButton(true)
        //  关闭计时器
        timer?.invalidate()
        timer = nil
        secondCount = 0
...
//            }, completion: nil)
    ...
    func videoRecordingTotolTime() {
        ...
        if secondCount == MaxVideoRecordTime {
            timer?.invalidate()
            let alertC = UIAlertController(title: "最长只能录制15秒钟", message: nil, preferredStyle: .Alert)
            alertC.addAction(UIAlertAction(title: "确定", style: .Cancel, handler: nil))
            self.presentViewController(alertC, animated: true, completion: nil)
            self.addBoFangButton(true)
            defineButton.backgroundColor = UIColor.redColor()
            defineButton.userInteractionEnabled = true
            self.onClickedEndButton(UIButton())
        }
    ...
    func defineButtonAction(){
        var image = UIImage()
        let videoAsset = AVURLAsset(URL: self.videoUrls)
        ...
        self.myFunc!(editedText: self.videoUrls,images:image)
    ...
    //播放按钮是否显示
    func addBoFangButton(isShow:Bool){
        if isShow{
            self.boFangButton.hidden = false
        }else{
            self.boFangButton.hidden = true
        }
    }
